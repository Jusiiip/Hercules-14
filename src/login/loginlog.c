...
void login_gepard_log(int fd, uint32 ip, const char* username, int rcode, const char* message)
{
	...
	retcode = SQL->Query(loginlog->sql_handle,
		...
		loginlog->dbs->log_login_db, sockt->ip2str(ip,NULL), esc_username, rcode, unique_id, esc_message);
...
{
	if (SQL_SUCCESS != SQL->Query(loginlog->sql_handle, "SHOW COLUMNS FROM `loginlog` LIKE 'unique_id'"))
	{
		Sql_ShowDebug(loginlog->sql_handle);
	...
	if (SQL->NumRows(loginlog->sql_handle) == 0)
	{
		if (SQL_SUCCESS != SQL->Query(loginlog->sql_handle, "ALTER TABLE `loginlog` ADD `unique_id` INT(11) UNSIGNED NOT NULL DEFAULT '0';"))
		{
			Sql_ShowDebug(loginlog->sql_handle);
			SQL->Free(loginlog->sql_handle);
			exit(EXIT_FAILURE);

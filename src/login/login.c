...
// (^~_~^) Gepard Shield Start
	...
	login_gepard_log(fd, sockt->host2ip(acc.last_ip), acc.userid, 100, "PIN Code check failed");
...
// (^~_~^) Gepard Shield Start
	...
	account_gepard_update_last_unique_id(sd->account_id, sockt->session[fd]->gepard_info.unique_id);
...
// (^~_~^) Gepard Shield Start
	...
	login_gepard_log(fd, ip, sd->userid, result, error);
...
// (^~_~^) Gepard Shield Start
		...
		if (account_gepard_check_license_version(sockt->session[fd], fd, sd->group_id))
		{
			return 0;
		}
...
// (^~_~^) Gepard Shield Start
	...
	login_gepard_log(fd, sockt->session[fd]->client_addr, sd->userid, 100, message);

...
void account_gepard_init(AccountDB* self)
{
	...
	if (SQL_SUCCESS != SQL->Query(db_sql->accounts, 
	...
	if (SQL_SUCCESS != SQL->Query(db_sql->accounts, "SELECT `version` FROM `gepard_min_allowed_license_version`"))
	{
		Sql_ShowDebug(db_sql->accounts);
		exit(EXIT_FAILURE);
	}

	if (SQL_SUCCESS != SQL->NextRow(db_sql->accounts))
	{
		if (SQL_SUCCESS != SQL->Query(db_sql->accounts, "INSERT INTO `gepard_min_allowed_license_version` VALUES ('0')"))
		{
			Sql_ShowDebug(db_sql->accounts);
			exit(EXIT_FAILURE);
		}
	...
	if (SQL_SUCCESS != SQL->Query(db_sql->accounts, "SHOW COLUMNS FROM `login` LIKE 'last_unique_id'"))
	{
		Sql_ShowDebug(db_sql->accounts);
		exit(EXIT_FAILURE);
	}

	if (SQL->NumRows(db_sql->accounts) == 0)
	{
		if (SQL_SUCCESS != SQL->Query(db_sql->accounts, "ALTER TABLE `login` ADD `last_unique_id` INT(11) UNSIGNED NOT NULL DEFAULT '0';"))
		{
			Sql_ShowDebug(db_sql->accounts);
			exit(EXIT_FAILURE);
		}
	...
	if (SQL_SUCCESS != SQL->Query(db_sql->accounts, "SHOW COLUMNS FROM `login` LIKE 'blocked_unique_id'"))
	{
		Sql_ShowDebug(db_sql->accounts);
		exit(EXIT_FAILURE);
	}
	...
	if (SQL->NumRows(db_sql->accounts) == 0)
	{
		if (SQL_SUCCESS != SQL->Query(db_sql->accounts, "ALTER TABLE `login` ADD `blocked_unique_id` INT(11) UNSIGNED NOT NULL DEFAULT '0';"))
		{
			Sql_ShowDebug(db_sql->accounts);
			exit(EXIT_FAILURE);
		}
...
int account_gepard_check_license_version(struct socket_data* s, int fd, int group_id)
{
	...
	else if (SQL_SUCCESS != SQL->NextRow(db_sql->accounts))
	{
		if (SQL_SUCCESS != SQL->Query(db_sql->accounts, "INSERT INTO `gepard_min_allowed_license_version` VALUES ('0')"))
		{
			Sql_ShowDebug(db_sql->accounts);
	...
	else
	{
		...
		SQL->GetData(db_sql->accounts, 0, &data, NULL);

		min_allowed_license_version = (unsigned int)strtoul(data, NULL, 10);

		SQL->FreeResult(db_sql->accounts);
...
void account_gepard_update_last_unique_id(int account_id, unsigned int unique_id)
{
	...
	if (SQL_SUCCESS != SQL->Query(db_sql->accounts, "UPDATE `login` SET `last_unique_id`= '%u' WHERE `account_id` = '%d'", unique_id, account_id))
	{
		Sql_ShowDebug(db_sql->accounts);
	}
	else if (SQL_SUCCESS == SQL->NextRow(db_sql->accounts))
...
bool account_gepard_check_unique_id(int fd, struct socket_data* s)
{
	...
	if (SQL_SUCCESS != SQL->Query(db_sql->accounts, "SELECT `unban_time`, `reason` FROM `gepard_block` WHERE `unique_id` = '%u'", unique_id))
	{
		Sql_ShowDebug(db_sql->accounts);
	...
	else if (SQL_SUCCESS == SQL->NextRow(db_sql->accounts))
	{
		...
		if (time_server <= time_unban)
		{
			...
			SQL->GetData(db_sql->accounts,  1, &data, NULL);
		...
		else if (SQL_ERROR == SQL->Query(db_sql->accounts, "DELETE FROM `gepard_block` WHERE `unique_id` = '%u'", unique_id))
		{
			Sql_ShowDebug(db_sql->accounts);
		}
	}

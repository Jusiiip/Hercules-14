...
{
	return sockt->session[fd]->gepard_info.unique_id;
}
 
struct socket_data* gepard_get_socket_data(int fd)
{
	return sockt->session[fd];
}
...
// (^~_~^) Gepard Shield Start
					...
					if (cmd != CS_GEPARD_SYNC && cmd != CS_GEPARD_INIT_ACK)
					{
						ShowError("Skip wrong packet id: 0x%04X\n", cmd);
						Assert_report(0);
					}
...
// (^~_~^) Gepard Shield Start
	...
	if (cmd == SC_GEPARD_INIT || cmd == SC_GEPARD_INFO || cmd == SC_GEPARD_SETTINGS)
	{
		return;
	}

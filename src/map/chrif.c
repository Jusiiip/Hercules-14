...
int chrif_gepard_req_block(unsigned int unique_id, const char* violator_name, unsigned int violator_aid, const char* initiator_name, unsigned int initiator_aid, const char* unban_time_str, const char* reason_str)
{
	...
	WFIFOHEAD(chrif->fd, offset);
	memcpy((void*)WFIFOP(chrif->fd, 0), send_buffer, offset);
	WFIFOSET(chrif->fd, offset);
...
bool chrif_gepard_ack_block(int fd)
{
	...
	safestrncpy(unban_time_str, (const char*)RFIFOP(fd, offset), GEPARD_TIME_STR_LENGTH);
	offset += GEPARD_TIME_STR_LENGTH;

	safestrncpy(reason_str, (const char*)RFIFOP(fd, offset), GEPARD_REASON_LENGTH);
	offset += GEPARD_REASON_LENGTH;

	safestrncpy(result_str, (const char*)RFIFOP(fd, offset), GEPARD_RESULT_STR_LENGTH);
	offset += GEPARD_RESULT_STR_LENGTH;

	if (violator_aid != 0 && (sd = map->id2sd(violator_aid)) != NULL)
	{
		...
		{
			if (gepard_get_unique_id(sd->fd) == unique_id)
			{
				gepard_send_info(sd->fd, GEPARD_INFO_BANNED, message_info);
				s->recv_crypt.pos_1 = rand() % 255;
				s->recv_crypt.pos_2 = rand() % 255;
				s->recv_crypt.pos_3 = rand() % 255;
			}
		}
	...
	if (initiator_aid != 0 && (sd = map->id2sd(initiator_aid)) != NULL)
	{
		clif->message(sd->fd, result_str);
	}

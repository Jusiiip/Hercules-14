...
ACMD(go)
{
	...
	const struct {
		...
		{ MAP_MORROC,      157,  90, 4 }, //  1 = Morroc
		{ MAP_GEFFEN,      119,  68, 3 }, //  2 = Geffen
		{ MAP_PAYON,       160, 231, 3 }, //  3 = Payon
		{ MAP_ALBERTA,      29, 234, 3 }, //  4 = Alberta
#ifdef RENEWAL
		{ MAP_IZLUDE,      128, 146, 3 }, //  5 = Izlude (Renewal)
#else
		{ MAP_IZLUDE,      128, 114, 3 }, //  5 = Izlude
#endif
		{ MAP_ALDEBARAN,   140, 119, 3 }, //  6 = Aldebaran
		{ MAP_LUTIE,       147, 134, 3 }, //  7 = Lutie
		{ MAP_COMODO,      205, 143, 3 }, //  8 = Comodo
		{ MAP_YUNO,        157, 183, 3 }, //  9 = Juno
		{ MAP_AMATSU,      110, 148, 3 }, // 10 = Amatsu
		{ MAP_GONRYUN,     160, 120, 3 }, // 11 = Kunlun
		{ MAP_UMBALA,       89, 157, 3 }, // 12 = Umbala
		{ MAP_NIFLHEIM,     21, 153, 3 }, // 13 = Niflheim
		{ MAP_LOUYANG,     217, 102, 3 }, // 14 = Luoyang
		{ MAP_NOVICE,       53, 111, 3 }, // 15 = Training Grounds
		{ MAP_JAIL,         23,  61, 3 }, // 16 = Prison
		{ MAP_JAWAII,      193, 239, 3 }, // 17 = Jawaii
		...
		{ MAP_VEINS,       216, 128, 3 }, // 24 = Veins
		{ MAP_VEIL,   	   119, 104, 3 }, // 25 = Veil
		{ MAP_MARKET,       31, 200, 3 }, // 26 = Market
	};
	...
	if (town < 0 || town >= ARRAYLENGTH(data)) {
		...
		if (strncmpi(map_name, "morroc", 4) == 0) { // Correct town name for 'morocc'
			...
			town = 23;
		} else if (strncmpi(map_name, "maintown", 3) == 0) { // Original town name for 'Town of Beginning'
			town = 25;	
		} else if (strncmpi(map_name, "market", 3) == 0) { // Original town name for 'market'
			town = 26;	
		}
	}
...
ACMD(gepard_block_nick)
{
	...
	violator_sd = map->nick2sd(violator_name, true);

	if (violator_sd != NULL)
	{
		violator_account_id = violator_sd->status.account_id;
		violator_unique_id = gepard_get_unique_id(violator_sd->fd);
	}
